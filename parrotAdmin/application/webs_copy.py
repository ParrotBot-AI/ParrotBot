...
#
#                 # 记录房间的账号
#                 self.user_presence[self.room_name] = True
#         except InvalidSignatureError:
#             await self.disconnect(None)
#
#     def user_has_returned(self, room_name):
#         # Check if the user is marked as present in the user_presence dictionary
#         if room_name in self.user_presence:
#             if self.user_presence[room_name]:
#                 return True
#         return False
#
#     async def disconnect(self, close_code):
#         # Leave room group
#         # sheet_id = int(self.room_name.split("_")[-1])
#         await self.channel_layer.group_discard(self.room_name, self.channel_name)
#         await self.channel_layer.group_discard("dvadmin", self.channel_name)
#         self.user_presence[self.room_name] = False
#
#         print("Connection closed")
#         # Wait for 8 seconds asynchronously
#
#         # await asyncio.sleep(8)
#         if not self.user_has_returned(self.room_name):
#             try:
#                 await self.close(close_code)
#                 # admin = AdminStream()
#                 # response = admin.pause_sheet(sheet_id=int(sheet_id))
